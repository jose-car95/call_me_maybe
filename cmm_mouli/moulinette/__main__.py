...
from moulinette.extract_functions_info import generate_function_calling_definition
from moulinette.generate_tests_and_corrections import (
    generate_function_calling_corrections,
...
class Moulinette:
    ...
    def prepare_exercises(
        ...
        functions_definition_path = input_dir / "function_definitions.json"
        ...
        with open(function_calling_corrections_path, "w") as handle:
            json.dump(
    ...
    def grade_student_answers(
        ...
        Args:
            student_answer_path: Path to student's function_calling_results.json file
        ...
        corrections = [
            ...
            for correction in generate_function_calling_corrections(filtered_exercises)
        ]
        ...
        for i, (student_answer, correction) in enumerate(zip(student_answers, corrections), 1):
            ...
            fn_name = student_answer.get("fn_name", student_answer.get("name"))
            fn_params = student_answer.get("args", student_answer.get("parameters", {}))

            if fn_name == UNABLE_TO_RETRIEVE_FN_NAME:
                if correction["fn_name"] == UNABLE_TO_RETRIEVE_FN_NAME:
                    total_score += 1
                    self.output.test_result(True)
                    continue
                self.output.expected("Expected function", correction["fn_name"])
                self.output.actual("Student called", fn_name)
                self.output.test_result(False, "fallback used on solvable prompt")
                continue
            ...
            try:
                ...
                self.output.expected("Expected parameters", correction["args"])
            ...
            if student_output != correction["expected_output"]:
                ...
                self.output.expected("Expected call", f"{correction['fn_name']}({correction['args']})")
